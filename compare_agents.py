...
obs = env.reset()
while(1):
    x, cost_map, goal, prev = agent2.get_cost_goal(obs, env.state, env)
...
v, w = env.getVelocity()
x, cost_map, goal, _ = agent2.get_cost_goal(obs, state, [v, w], env.config.scale)
...
for t in count():
    ...
    v, w = env.getVelocity()
    x, cost_map, goal, _ = agent2.get_cost_goal(next_obs, next_state, [v, w], env.config.scale)
...
height = env.height_map
center = np.array([*height.shape, 0])/2
goal = goal/env.config.scale + center
traj1 = traj1/env.config.scale + center
traj2 = traj2/env.config.scale + center
